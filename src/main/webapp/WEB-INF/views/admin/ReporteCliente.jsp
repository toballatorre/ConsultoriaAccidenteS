...
<body>
	...
	<table class="table table-striped table-hover">
		<thead class="thead-light">
			<tr>
				<th>Cliente: #<c:out value="${c.getId()}"/> <c:out value="${c.getNombreEmpresa()}"/> </th>
				<th> RUT: <c:out value="${c.getRutEmpresa()}"/> </th>
			</tr>
		</thead>
	</table>
	...
	<table class="table table-striped table-hover">
		<thead class="thead-light">
			<tr><th colspan="6"> REPORTES DE ACCIDENTES </th></tr>
			<tr>
				<th> # </th>
	...
	<table class="table table-striped table-hover">
		<thead  class="thead-light">
			<tr><th colspan="6"> REPORTE DE ASESORIAS ESPECIALES </th></tr>
			<tr>
				<th> # </th>
		...
		<tbody>
			<c:forEach items="${c.getListaAsesoriaE()}" var="ae">
				<tr>
					...
					<fmt:formatDate value="${ae.getFechasolicitud() }" pattern="dd/MM/yyyy" var="fechaSol"/>
					<td> <c:out value="${fechaSol}"/> </td>
					<td> <c:out value="${ae.getMotivo() }"/> </td>
					<td> <c:out value="${ae.getPreferenciaHorario() }"/> </td>
					<fmt:formatDate value="${ae.getFechaagenda() }" pattern="dd/MM/yyyy" var="fechaAg"/>
	...
	<table class="table table-striped table-hover">
		<thead  class="thead-light">
			<tr><th colspan="5"> REPORTE FACTURA </th></tr>
			<tr>
				<th> # </th>
				<th> Fecha Emision </th>
		...
		<tbody>
			<c:forEach items="${c.getListaFacturas()}" var="f">
				<tr>
					...
					<fmt:formatDate value="${f.getFechaEmision()}" pattern="dd/MM/yyyy" var="fechaEm"/>
					<td> <c:out value="${fechaEm}"/> </td>
					<fmt:formatDate value="${f.getFechaVencimiento()}" pattern="dd/MM/yyyy" var="fechaVe"/>
					<td> <c:out value="${fechaVe}"/> </td>
					<fmt:formatDate value="${f.getFechaPago()}" pattern="dd/MM/yyyy" var="fechaPag"/>
	...
	<table class="table table-striped table-hover">
		<thead  class="thead-light">
			<tr><th colspan="6"> REPORTE CAPACITACIONES </th></tr>
			<tr>
				<th> # </th>
				<th> Tema </th>
				<th> Objetivos </th>
		...
		<tbody>
			<c:forEach items="${c.getListaCapacitaciones()}" var="cap">
				<tr>
					...
					<fmt:formatDate value="${cap.getFecha()}" pattern="dd/MM/yyyy" var="fechaCap"/>
